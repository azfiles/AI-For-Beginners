...
def current_code_hash(source):
    path = ROOT / source
    if not path.is_file() or path.suffix != ".ipynb":return None
...
def notebook_status(source):
    ...
    if record.get("full_status") == "passed" and record.get("code_sha256") == current_code_hash(source):
    ...
    if source in SHORT_VERIFIED or record.get("short_test"):
        ...
        return ("verified", "已验证可运行（短训练）", "完整训练复验中 · "+scope)
...
def shell(name,body,source=""):
    ...
    return '<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="Microsoft AI-For-Beginners 中文课程与代码实践"><title>'+html.escape(name)+' · AI 入门</title><link rel="stylesheet" href="/style.css"><link rel="stylesheet" href="/learning.css"><link rel="icon" href="/favicon.svg"><link rel="stylesheet" href="/vendor/katex/katex.min.css"><script defer src="/vendor/katex/katex.min.js"></script><script defer src="/vendor/katex/contrib/auto-render.min.js"></script><script defer src="/math.js"></script><script defer src="/learning.js"></script></head><body data-source="'+html.escape(source,quote=True)+'" data-page-title="'+html.escape(name,quote=True)+'" data-total-lessons="'+str(len(rows))+'" data-course-page="'+('true' if aliases.get(source,source) in {key for _,_,key in rows} else 'false')+'"><header><a class="brand" href="/"><b>AI</b> 入门 · 中文学习站</a><div class="header-actions"><button class="learning-open" id="learning-open" type="button" aria-controls="learning-drawer"><strong>学习记录</strong><span>0</span></button><a href="'+REPO+'">GitHub 源码 ↗</a></div></header><div class="shell"><nav aria-label="课程导航">'+nav+'</nav><main>'+body+'<footer>课程 © Microsoft · <a href="/LICENSE.txt">MIT 许可证</a> · 中文来自上游 Co-op Translator，原文为准。<br>Notebook 注释保留原语言。来源版本 '+REV[:8]+'</footer></main></div></body></html>'
...
for filename in ("style.css","learning.css","math.js","learning.js","favicon.svg"):shutil.copyfile(ROOT/"website"/filename,OUT/filename)
