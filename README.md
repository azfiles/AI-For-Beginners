...
课程页面提供按登录用户保存的个人学习功能：选中正文段落可保存引用和文字笔记，也可将课程标记为已完成，并在「学习记录」抽屉中查看总体进度。Site 使用 ChatGPT 登录提供的稳定用户 ID 和 D1 数据库隔离记录，同一用户可跨浏览器、跨设备继续学习；首次升级会自动合并旧版浏览器本地记录，并保留 JSON 导入/导出备份。
